...
#          Then estimate the changes drdot and decc needed in SGRID pars
#          BNSdata_rdot and BNSdata_ecc to reduce the measured eccentricity ecc.
#
...
parser = argparse.ArgumentParser(description=
    ...
    BNSdata_rdot and BNSdata_ecc to reduce the measured eccentricity ecc.''',
    epilog='''Example:
    EccRed.py bam0/moving_puncture_distance.lxyz4
...
##################################################################
# calc ecc, drdot, decc from fit
def eval_pars(S0, A0,A1,B, omega,phi,  r0, Omega):
    ecc = -B/(omega*r0)
    drdot = -B*np.sin(phi)
    decc  = B*omega*np.cos(phi)/(Omega**2.0 * r0)
    return ecc, drdot, decc
...
ecc, drdot, decc = eval_pars(S0, A0,A1,B, omega,phi,  d0, Omega)
print("# ecc =", ecc)
print("# drdot =", drdot)
print("# decc  =", decc)
...
ecc, drdot, decc = eval_pars(S0, A0,A1,B, omega,phi,  r0, Omega)
print("# ecc =", ecc)
print("# drdot =", drdot)
print("# decc  =", decc)
